...
_EXTRACTION_SIGNATURE_KEY = "morph_extraction_signature"
...
def cache_anki_data(
    ...
    performance gain. The morphs a previous recalc extracted are re-used unless
    something they depend on changed, see _cached_morphs_are_reusable.
    """
    assert mw is not None

    am_db = AnkiMorphsDB()
    am_db.create_recalc_state_table()
    ...
    # The signature is cleared before the first write and only written again
    # after the last one. A recalc that is cancelled or crashes in between
    # therefore leaves no signature, and the next one rebuilds from scratch
    # instead of trusting a half-updated database.
    am_db.clear_recalc_state(_EXTRACTION_SIGNATURE_KEY)

    # Every recalc has always started the "morphs seen today" over, and the rest
    # of the database surviving one does not change that.
    ...
    # Cards nothing reads anymore, and cards that were just extracted again,
    # both still hold the morphs of the previous recalc. When the tables were
    # dropped above there is nothing left to clear.
    ...
    am_db.set_recalc_state(_EXTRACTION_SIGNATURE_KEY, extraction_signature)
    am_db.con.close()
...
def _cached_morphs_are_reusable(
    ...
    if am_db.get_recalc_state(_EXTRACTION_SIGNATURE_KEY) != extraction_signature:
        return False
...
def _every_card_has_one_source_of_morphs(
    ...
    """
    Note filters may overlap, which is fine as long as the overlapping ones read
    the same field with the same morphemizer. When they don't, a card's morphs
    are the union of several sources, which one hash per card cannot describe.
    """
...
def _get_expression_hash(config_filter: AnkiMorphsConfigFilter, expression: str) -> int:
    """
    The hash covers the filter the text was read through and not only the text
    itself: a note that changes note type keeps its text but can land under a
    filter with a different morphemizer, and morphs from the wrong tokenizer
    would otherwise be re-used.

    Returns a signed integer because that is what sqlite stores.
    """
    source = (
...
def _extract_and_assign_morphs(
    ...
    Cards whose expression hash still matches keep the morphs a previous recalc
    extracted: they are left out of the batch and their 'morphs' stays None.
