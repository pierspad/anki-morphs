...
from ankimorphs.recalc import caching
...
def test_a_database_from_an_older_ankimorphs_version_is_rebuilt(  # pylint:disable=unused-argument
    ...
    am_db = AnkiMorphsDB()
    with am_db.con:
        am_db.con.execute("DROP TABLE IF EXISTS Recalc_State")
...
def test_an_interrupted_recalc_does_not_leave_a_cache_that_is_believed_valid(  # pylint:disable=unused-argument
    ...
    am_db = AnkiMorphsDB()
    signature = am_db.get_recalc_state(caching._EXTRACTION_SIGNATURE_KEY)
    am_db.con.close()
